...
def scrape_weather():
    ...
    cast = summary.get_text()
    # 현재 OO℃  (최저 OO˚ / 최고 OO˚)
    curr_temp = summary.find("span", attrs={"class":"temperature"}).get_text().replace("높아요", "") # 현재 온도
    ...
    weather = soup.find_all("div", attrs={"class":"day_data"})[0].find_all("span", attrs={"class":"rainfall"})
    morning_rain_rate = weather[0].get_text().strip() # 오전 강수확률
    afternoon_rain_rate = weather[1].get_text().strip() # 오후 강수확률
